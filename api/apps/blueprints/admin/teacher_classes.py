...
def create_teacher_class():
    ...
    teacher_class_service = TeacherClassService()
    result = teacher_class_service.create_teacher_class(teacher_id, class_id)  # 修改这行
    
    current_app.logger.info(f"成功创建教师班级关联: 教师ID {teacher_id}, 班级ID {class_id}")
...
def update_teacher_class(teacher_id: int, class_id: int):
    data, error = validate_json_input(['new_teacher_id'])
    if error:
        return error
    ...
    if not new_teacher_id:
        return error_response("新教师ID不能为空", 400)
