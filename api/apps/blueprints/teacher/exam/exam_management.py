...
def create_exam(teacher_id):
    ...
    try:
        ...
        current_app.logger.info(f"Teacher {teacher_id} created exam: {exam}")
...
def get_exams(teacher_id):
    ...
    try:
        ...
        # 创建考试服务实例并获取考试列表
        exam_service = ExamService()
        exams_data = exam_service.get_exams_by_teacher(teacher_id, page, per_page)
        
        current_app.logger.info(f"Teacher {teacher_id} retrieved exams list")
...
def get_exam(teacher_id, exam_id):
    """
    获取考试详情
    ...
    try:
        # 创建考试服务实例并获取考试详情
        exam_service = ExamService()
        exam = exam_service.get_exam_by_id_and_teacher(exam_id, teacher_id)
...
def update_exam(teacher_id, exam_id):
    """
    更新考试信息
...
def delete_exam(teacher_id, exam_id):
    """
    删除考试
    ...
    try:
        # 创建考试服务实例并删除考试
        exam_service = ExamService()
        result = exam_service.delete_exam(exam_id, teacher_id)
