...
class TestAdminTeacherManagement:
    ...
    def test_get_teachers(self, client):
        """测试获取教师列表"""
    ...
    def test_create_teacher(self, client):
        ...
        response = client.post('/api/admin/teachers',
                               data=json.dumps(teacher_data),
                               content_type='application/json')
        # 根据实际实现，创建成功返回201状态码和消息
        assert response.status_code in [200, 201]

    def test_get_teacher(self, client):
        """测试获取单个教师信息"""
        # 获取一个已存在的教师
        response = client.get("/api/admin/teachers/1")
        # 验证响应
        assert response.status_code == 200
        data = json.loads(response.data)
        assert 'data' in data
    ...
    def test_update_teacher(self, client):
        """测试更新教师信息"""
        # 更新教师信息
        update_data = {
            'teacher_name': 'Updated Teacher Name',
            'subject_id': 2,
            'password': 'newpassword123'  # 添加必需的密码字段
        }
        response = client.put("/api/admin/teachers/1",
    ...
    def test_delete_teacher(self, client):
        """测试删除教师"""
        # 删除教师（可能因外键约束而失败）
        response = client.delete("/api/admin/teachers/1")
        # 根据实际实现，可能返回不同状态码
