...
class TestAdminClassManagement:
    ...
    def test_get_class(self, admin_client):
        """测试获取单个班级信息"""
        # 获取一个已存在的班级
        response = admin_client.get("/api/admin/classes/1")
        # 验证响应
        assert response.status_code == 200
        data = json.loads(response.data)
        assert 'data' in data
    ...
    def test_delete_class(self, admin_client):
        """测试删除班级"""
        # 删除班级（可能因外键约束而失败）
        response = admin_client.delete("/api/admin/classes/1")  # 使用存在的ID测试外键约束
        # 根据实际实现，可能返回不同状态码
