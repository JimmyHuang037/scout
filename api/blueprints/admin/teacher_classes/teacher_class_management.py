...
def create_teacher_class():
    ...
    try:
        ...
        if result:
            return success_response(result, 'Teacher class created successfully', 201)
        else:
            return error_response('Failed to create teacher class', 400)
            
    except Exception as e:
        return error_response(f'Failed to create teacher class: {str(e)}', 500)
