...
def get_teacher_classes():
    ...
    try:
        ...
        current_app.logger.info("Admin retrieved teacher-class associations")
        return success_response(result)
        
    except Exception as e:
        current_app.logger.error(f'Failed to fetch teacher-class associations: {str(e)}')
...
def create_teacher_class():
    ...
    try:
        ...
        if not all([teacher_id, class_id]):
            current_app.logger.warning("Create teacher-class association attempt with missing fields")
        ...
        if result:
            current_app.logger.info(f"Admin created teacher-class association: teacher_id={teacher_id}, class_id={class_id}")
            ...
            current_app.logger.error("Failed to create teacher-class association")
...
def get_teacher_class(teacher_id):
    ...
    try:
        ...
        current_app.logger.info(f"Admin retrieved teacher-class associations for teacher_id={teacher_id}")
        return success_response(result)
    ...
    try:
        ...
        if delete_result:
            ...
            if create_result:
                ...
                current_app.logger.error("Failed to create new teacher-class association during update")
                return error_response('Failed to update teacher-class association', 400)
        else:
            current_app.logger.error("Failed to delete old teacher-class association during update")
            return error_response('Failed to update teacher-class association', 400)
...
def delete_teacher_class(teacher_id):
    ...
    try:
        ...
        if not class_id:
            current_app.logger.warning("Delete teacher-class association attempt with missing class_id")
        ...
        if result:
            current_app.logger.info(f"Admin deleted teacher-class association: teacher_id={teacher_id}, class_id={class_id}")
            return success_response(result, 'Teacher-class association deleted successfully')
        else:
            current_app.logger.error("Failed to delete teacher-class association")
