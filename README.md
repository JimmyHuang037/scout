...
在 `api/runtime/` 目录中存储了应用运行时生成的数据：

1. **flask_session/** - 存储Flask会话文件
   - 由Flask-Session扩展自动生成和管理
   - 包含用户会话信息
   - 文件内容为无扩展名的会话数据文件
...
这些目录中的内容为运行时动态生成的临时数据，不应提交到版本控制系统中。
...
将运行时数据集中存储在 `runtime/` 目录中的优势：

1. **更好的组织性**：所有运行时数据集中管理
2. **易于维护**：便于备份、清理和权限管理
3. **清晰的目录结构**：区分代码文件和运行时数据
4. **简化忽略规则**：只需忽略整个runtime目录即可排除所有运行时数据
