...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>添加学生</h2>
    <mat-dialog-content>
      <form [formGroup]="studentForm" (ngSubmit)="onSubmit()">
        <mat-form-field appearance="fill" class="full-width">
          <mat-label>学生ID</mat-label>
          <input matInput formControlName="student_id" required>
          <mat-error *ngIf="studentForm.get('student_id')?.invalid && studentForm.get('student_id')?.touched">
            学生ID是必填项
          </mat-error>
        </mat-form-field>
...
export class CreateStudentDialogComponent implements OnInit {
  ...
  onSubmit(): void {
    if (this.studentForm.valid) {
      ...
      if (selectedClass) {
        student.class_name = selectedClass.class_name;
      }
